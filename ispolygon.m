...
%使用者在螢幕上用滑鼠左鍵依序輸入點，按滑鼠右鍵停止輸入，程式會判斷將點依序連起來的圖形是否為多邊形。
function ispolygon
    X=[];
    Y=[];
    pall=[];
    ...
    %以下檢測圖形是凹或凸多邊形，依序紀錄向量(1,2)、(2,3),...,(n,1)，再檢測相鄰向量的determinant正負有無變動，有變則為凹多邊形。
    for ii=1:(size(X,2)-1)
    ...
    for ii=1:(size(X,2)-1)
        ...
        point_combination=[point_combination [ii;ii+1]];       %蝝紀錄多項式是由哪兩個點形成的
        pall=[pall;p];                                         
    end
